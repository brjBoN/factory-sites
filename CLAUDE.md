...
- REPO HYGIENE: never `git add -A` blindly — verify every modified file is one you actually
  edited this round. On 2026-07-24 a stale editor buffer reverted CLAUDE.md (wiping the day's
  protocol additions) and `add -A` shipped it; caught by diff-stat review, restored from git.
  If you keep CLAUDE.md open in an editor, reload it before saving.
